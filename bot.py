...
# ============================================================
#  STICKER MAKER — bikin stiker custom dari gambar upload
#  Endpoint: POST /guilds/{guild.id}/stickers (multipart)
#  Syarat: bot butuh izin CREATE_GUILD_EXPRESSIONS
# ============================================================

_STICKER_ENDPOINT = "https://discord.com/api/v10"

def _sanitize_sticker_name(raw: str, fallback: str) -> str:
    """Sticker name: 2-30 chars, alphanumeric + underscore only."""
    name = re.sub(r"[^a-zA-Z0-9_]", "_", raw).strip("_")
    name = name[:30] or fallback
    if len(name) < 2:
        name = (name + "_x" * 10)[:30]
        return name

def _prepare_sticker_bytes(raw: bytes) -> bytes:
    """Maksimal 320x320 & 512 KiB. Pakai Pillow; fallback raw kalau gagal."""
    try:
        ...
        img = img.convert("RGBA") if img.mode not in ("RGBA", "LA", "P") else img
        out = BytesIO()
        img.save(out, format="PNG")
        data = out.getvalue()
        # Kalau masih > 512 KiB (PNG gradient rame), turunin jadi palette 256 warna
        if len(data) > 512 * 1024:
            out2 = BytesIO()
            img.convert("P", palette=Image.ADAPTIVE, colors=256).save(out2, format="PNG")
            data = out2.getvalue()
        return data
    except Exception:
        # Pillow gagal / gak kebaca — kirim raw apa adanya, biar Discord yang reject
        return raw

@tree.command(name="stiker", description="Bikin stiker custom dari gambar dan simpen langsung ke server 🔥")
@app_commands.describe(
    gambar="Gambar yang mau jadi stiker (upload file)",
    nama="Nama stiker (2-30 karakter, huruf/angka/underscore)",
    tags="Tag pencarian stiker, pisah pakai koma",
)
async def slash_stiker(
    interaction: discord.Interaction,
    ...
    # Cek izin bot bikin stiker (CREATE_GUILD_EXPRESSIONS)
    if not interaction.app_permissions.create_expressions:
        await interaction.response.send_message(
            "❌ Bot gak punya izin **Create Expressions** di server ini. "
            "Kasih izin itu dulu (Server Settings → Roles → Seraphine → buka 'Create Expressions').",
            ephemeral=True,
        )
        return

    # Validasi file
    if '(image' not in (gambar.content_type or ''):
        await interaction.response.send_message("❌ Itu bukan gambar bro ('{0}'). Kirim file PNG / JPG / WEBP.".format(gambar.filename), ephemeral=True)
        return
    if gambar.size > 8 * 1024 * 1024:
        await interaction.response.send_message("❌ File kebesaran (maks 8 MB dari Discord).", ephemeral=True)
        return

    await interaction.response.defer(ephemeral=False)

    try:
        # Download & olah gambar
        raw = await gambar.read()
        image_bytes = _prepare_sticker_bytes(raw)

        # Nama stiker: dari param, fallback ke nama file (tanpa ekstensi)
        base = os.path.splitext(gambar.filename)[0]
        name = _sanitize_sticker_name(nama or base, base or "sticker")
        desc = f"Stiker dibuat oleh {interaction.user.name} via Seraphine"
        tag_str = tags.strip()[:200]
        ...
        files = {"file": (name + ".png", image_bytes, "image/png")}

        resp = requests.post(
            f"{_STICKER_ENDPOINT}/guilds/{interaction.guild.id}/stickers",
            headers=headers,
            data=payload,
            files=files,
            timeout=30,
        ...
        if resp.status_code == 200:
            sticker = resp.json()
            embed = discord.Embed(
                title="✅ Stiker Berhasil Dibuat!",
                description=f"Stiker **:{sticker.get('name', name)}:** udah disimpen ke server!",
                color=0x2ECC71,
                timestamp=datetime.now(),
            )
            embed.set_image(url=f"https://cdn.discordapp.com/stickers/{sticker['id']}.png")
            embed.add_field(name="🆔 ID", value=sticker.get("id", "?"), inline=True)
            embed.add_field(name="🔤 Nama", value=sticker.get("name", name), inline=True)
            if tag_str:
                embed.add_field(name="🏷️ Tags", value=tag_str, inline=False)
            await interaction.followup.send(embed=embed)
            logger.info(f"Sticker '{name}' created in guild {interaction.guild.id} by {interaction.user.name}")
        else:
            err = "Unknown"
            try:
                err = resp.json().get("message", str(resp.json())) if resp.headers.get("content-type", "").startswith("application/json") else resp.text[:300]
            except Exception:
                err = resp.text[:300]
            logger.error(f"Sticker create failed: {resp.status_code} {err}")
            await interaction.followup.send(
                f"❌ Gagal bikin stiker (HTTP {resp.status_code}): `{err}`\n"
                "Cek slot stiker masih cukup (default 5) & izin bot udah bener.",
                ephemeral=False,
            )
    except discord.Forbidden:
        await interaction.followup.send("❌ Bot gak punya izin Create Expressions di server ini.", ephemeral=True)
    except Exception as e:
        logger.error(f"Sticker error: {e}")
        await interaction.followup.send(f"❌ Ada error pas bikin stiker: `{e}`", ephemeral=True)
